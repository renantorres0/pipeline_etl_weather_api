...
def create_dataframe(path_name:str) -> pd.DataFrame:    
    ...
    logging.info(f"DataFrame created with {len(df)} records and {len(df.columns)} columns.")
    return df
...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info("Weather columns normalized and merged into the main DataFrame.")
    return df

def drop_columns(df: pd.DataFrame, columns_names: list[str]) -> pd.DataFrame:
    logging.info(f"Dropping columns: {columns_names}")
    df = df.drop(columns=columns_names)
    logging.info(f"Columns dropped: {columns_names}")
    return df
...
def rename_columns(df: pd.DataFrame, columns_names:dict[str, str]) -> pd.DataFrame:
    ...
    logging.info(f"Columns renamed: {columns_names}")
    return df
...
def normalize_datetime_columns(df: pd.DataFrame, columns_names: list[str]) -> pd.DataFrame:
    ...
    logging.info(f"Datetime columns normalized: {columns_names}")
    return df
